...
test("loadDesignSystemConfigFile returns undefined for missing, invalid, and malformed config files", async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "workspace-dev-design-system-load-"));
  ...
  try {
    ...
    assert.equal(logs.some((entry) => entry.includes("is invalid; using MUI defaults")), true);
    assert.equal(logs.some((entry) => entry.includes("Failed to load design system config")), true);
  } finally {
...
`,
  ...
  assert.match(transformed, /import \{ ExistingButton as ExistingButton2 \} from "@acme\/ui";/);
  assert.match(transformed, /import \{ FormField \} from "@acme\/forms";/);
  assert.match(transformed, /<ExistingButton2 disabled>Continue<\/ExistingButton2>/);
...
test("inferDesignSystemConfigFromProject infers the dominant library, ignores excluded directories, and derives MUI bases", async () => {
  const projectRoot = await mkdtemp(path.join(os.tmpdir(), "workspace-dev-design-system-scan-"));
  ...
  try {
    ...
    await mkdir(path.join(projectRoot, "node_modules", "ignored"), { recursive: true });
...
test("inferDesignSystemConfigFromProject honors a library override", async () => {
  const projectRoot = await mkdtemp(path.join(os.tmpdir(), "workspace-dev-design-system-override-"));
...
test("writeDesignSystemConfigFile writes sorted output and respects force", async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "workspace-dev-design-system-write-"));
...
`
    ...
    assert.equal(getDefaultDesignSystemConfigPath({ outputRoot: tempDir }), path.join(tempDir, "design-system.json"));
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
