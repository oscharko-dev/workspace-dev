...
function resolveProtectedWriteRoute(
  ...
  if (pathname === "/workspace/submit" || pathname === FIGMA_IMPORT_ROUTE) {
    return {};
  }
...
export function createWorkspaceRequestHandler({
  ...
  return async (
    ...
    try {
      ...
      if (method === "OPTIONS") {
        // CORS preflight for the Figma plugin import endpoint.
        if (pathname === FIGMA_IMPORT_ROUTE) {
          const requestOrigin =
          ...
          if (
            ...
            FIGMA_PLUGIN_ALLOWED_ORIGINS.has(requestOrigin)
          ) {
          ...
          // Non-Figma origin preflight — fall through to standard rejection.
        }
      ...
      if (method === "POST") {
        ...
        // Figma plugin import: cross-origin from figma.com, bypasses
        // same-origin validateWriteRequest but enforces its own origin check.
        if (pathname === FIGMA_IMPORT_ROUTE) {
          const requestOrigin =
            typeof request.headers.origin === "string"
              ? request.headers.origin
          ...
          // Content-Type check (same as validateWriteRequest).
          const contentType =
          ...
          if (
            ...
            sendValidationError({
              payload: {
                ...
                message:
                  "Figma import requires 'Content-Type: application/json'.",
              },
              fallbackMessage: "Figma import validation failed.",
            });
            return;
          }

          // Origin check: allow Figma plugin origins AND same-origin.
          if (!isFigmaOrigin) {
            const sameOriginCheck = validateWriteRequest({
              request,
              host,
            ...
            if (!sameOriginCheck.ok) {
              sendAuditedError({
                ...
                payload: {
                  ...
                  message:
                    "Figma import accepts requests from Figma plugins or same-origin clients only.",
                },
                event: "security.request.rejected_origin",
                level: "warn",
                fallbackMessage: "Figma import origin rejected.",
              });
              return;
            }
          }

          // Rate limit (shares the same limiter as submit).
          ...
          if (!rateLimitResult.allowed) {
            ...
            sendAuditedError({
              ...
              payload: {
                ...
                message: `Too many import requests. Retry after ${rateLimitResult.retryAfterSeconds} seconds.`,
              },
              event: "security.request.rate_limited",
              level: "warn",
              fallbackMessage: "Figma import rate limited.",
            });
            return;
          }
          ...
          await handleFigmaImport({
            request,
            response,
            requestOrigin: isFigmaOrigin ? requestOrigin : undefined,
            absoluteOutputRoot,
            jobEngine,
            sendValidationError,
            sendRequestFailure,
...
async function handleFigmaImport({
  ...
  if (!rawBody.ok) {
    if (rawBody.reason === "OVERSIZE") {
      sendValidationError({
        ...
        fallbackMessage: "Figma import payload too large.",
      });
    } else {
      sendValidationError({
        payload: {
  ...
  // Validate as clipboard envelope.
  if (!looksLikeClipboardEnvelope(payload)) {
    sendValidationError({
      payload: {
        error: ErrorCode.INVALID_PAYLOAD,
        message:
          "Request body must be a workspace-dev clipboard envelope (kind: workspace-dev/figma-selection@1).",
      },
      fallbackMessage: "Figma import validation failed.",
    });
    return;
  ...
  const envelopeResult = validateClipboardEnvelope(payload);
  if (!envelopeResult.valid) {
    const errorCode = isClipboardEnvelope(payload)
      ? ErrorCode.SCHEMA_MISMATCH
      : ErrorCode.UNSUPPORTED_CLIPBOARD_KIND;
    sendValidationError({
      payload: {
        error: errorCode,
        message: `Envelope validation failed: ${summarizeEnvelopeValidationIssues(envelopeResult.issues)}`,
      },
      fallbackMessage: "Figma import validation failed.",
    });
    return;
  ...
  // Normalize envelope → pipeline-compatible Figma file JSON.
  const normalized = normalizeEnvelopeToFigmaFile(envelopeResult.envelope);
  const normalizedJson = JSON.stringify(normalized);

  // Write to temp file for the local_json pipeline path.
  const importUUID = randomUUID();
  const importTempDir = path.join(absoluteOutputRoot, "tmp-figma-paste");
  const importTempPath = path.join(importTempDir, `${importUUID}.json`);
  try {
    await mkdir(importTempDir, { recursive: true });
    await writeFile(importTempPath, normalizedJson, "utf8");
  } catch (error) {
    sendRequestFailure({
      statusCode: 500,
      payload: {
        ...
        message: sanitizeErrorMessage({
          error,
          fallback: "Could not write import payload to disk.",
        }),
      },
      fallbackMessage: "Figma import failed.",
    });
    return;
  }

  // Submit the job via the existing local_json pipeline.
  const submitInput: Parameters<JobEngine["submitJob"]>[0] = {
    figmaSourceMode: "local_json",
    figmaJsonPath: importTempPath,
    enableGitPr: false,
  };

  let jobId: string;
  try {
    const accepted = jobEngine.submitJob(submitInput);
    jobId = accepted.jobId;
  } catch (error) {
    // Clean up temp file on submission failure.
    try {
      await unlink(importTempPath);
    } catch {
      /* best-effort */
    }
    ...
    if (
      ...
      sendRequestFailure({
        statusCode: 429,
        payload: {
          error: ErrorCode.QUEUE_BACKPRESSURE,
          message: sanitizeErrorMessage({
            error,
            fallback: "Job queue limit reached.",
          }),
        },
        fallbackMessage: "Figma import queue full.",
      });
      return;
    }
    sendRequestFailure({
      ...
      payload: {
        ...
        message: sanitizeErrorMessage({
          error,
          fallback: "Failed to submit import job.",
        }),
      },
      fallbackMessage: "Figma import failed.",
    });
  ...
  // Schedule temp file cleanup using the same polling strategy as figma_paste.
  scheduleFigmaPasteTempCleanup({
    jobEngine,
    jobId,
    filePath: importTempPath,
  });

  logAuditEvent({
    event: "workspace.submit.accepted",
    message: `Figma plugin import accepted: ${envelopeResult.envelope.selections.length} selection(s).`,
    jobId,
    statusCode: 202,
  });

  // Set CORS response header when the request came from a Figma plugin.
  ...
  sendJson({
    ...
    payload: {
      jobId,
      trackingUrl: `/workspace/jobs/${jobId}`,
      selections: envelopeResult.envelope.selections.length,
    },
  });
}
