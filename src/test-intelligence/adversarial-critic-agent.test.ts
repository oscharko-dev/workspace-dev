...
test("adversarial critic: negative coverage accounting passes saturated lift when the requested relative improvement is impossible", () => {
  const accounting = computeNegativeCoverageAccounting({
    baselineList: makeList([
...
test("adversarial critic: non-saturated suites fail when no additional negative case is created", () => {
  const accounting = computeNegativeCoverageAccounting({
    baselineList: makeList([
      { id: "tc-1", type: "negative" },
      { id: "tc-2", type: "functional" },
