...
test("Issue #1929: runFigmaToQcTestCases still emits faithfulness artifacts when the initial logic verdict is reject", async () => {
  ...
  try {
        ...
        if (request.responseSchemaName === "workspace-dev-logic-judge-v1") {
          return {
            ...
            content: {
              ...
              repairInstructions: [
                {
                  testCaseId: SAMPLE_DRAFT.title,
                  path: "steps[0]",
                  instruction:
                    "Adjust the generated case to satisfy the logic judge.",
                },
              ],
            },
            finishReason: "stop" as const,
    ...
    assert.equal(result.logicJudge?.verdict.verdict, "reject");
    assert.equal(result.faithfulnessJudge?.verdict.verdict, "reject");
    assert.ok(result.artifactPaths.faithfulnessJudgeVerdict);
    await stat(
      path.join(result.artifactDir, "agent-role-runs", "faithfulness_judge.json"),
    );
    assert.equal(client.callCount(), 2);
    ...
    assert.equal(result.repairLoop?.iterations[0]?.faithfulnessVerdict, "reject");
  } finally {
    globalThis.fetch = originalFetch;
    await rm(tempRoot, { recursive: true, force: true });
  }
