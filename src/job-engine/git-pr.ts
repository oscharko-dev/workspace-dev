...
export const runGitPrFlowWithDeps = async ({
  ...
  const branchSeed = (importSessionId ?? jobId).replace(/[^a-zA-Z0-9._-]+/g, "-");
  const branchSuffix = branchSeed.slice(0, 24) || jobId.slice(0, 8);
  ...
  try {
    ...
    const branchName = `auto/figma/${boardKey}-${branchSuffix}`;
    ...
    const prResponse = await fetchImpl(`https://api.github.com/repos/${githubRepo.owner}/${githubRepo.name}/pulls`, {
      ...
      body: JSON.stringify({
        ...
        body: generationDiff
          ? `Generated by workspace-dev job ${jobId}.${importSessionId ? ` Import session: ${importSessionId}.` : ""}\n\n${formatDiffForPrDescription(generationDiff)}`
          : `Generated by workspace-dev job ${jobId}.${importSessionId ? ` Import session: ${importSessionId}.` : ""}`,
        head: branchName,
        base: defaultBranch
      }),
