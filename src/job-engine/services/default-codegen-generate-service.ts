...
const GENERATED_APP_PLAYWRIGHT_SPEC_PATH = "e2e/template.spec.ts";
...
const createAppPlaywrightSpecFile = (sourceName: string): GeneratedFile => ({
  path: GENERATED_APP_PLAYWRIGHT_SPEC_PATH,
...
test(${escapeString(`renders generated ${sourceName || "workspace"} app in a real browser`)}, async ({ page }) => {
  await page.goto("/");

  await expect(page.getByTestId("generated-app")).toBeVisible();
});

test(${escapeString(`captures visual audit evidence for the generated ${sourceName || "workspace"} app`)}, async ({ page }, testInfo) => {
...
export const DefaultCodegenGenerateService: StageService<CodegenGenerateStageInput> = {
  ...
  execute: async (input, context) => {
    ...
    generatedFiles.push(createAppPlaywrightSpecFile(ir.sourceName));
