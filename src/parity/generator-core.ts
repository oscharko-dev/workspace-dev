...
interface SemanticInputModel {
  labelNode?: ScreenElementIR;
  valueNode?: ScreenElementIR;
  labelIcon?: SemanticIconModel;
  suffixText?: string;
  suffixIcon?: SemanticIconModel;
  isSelect: boolean;
}
...
const splitTextRows = (texts: ScreenElementIR[]): { topRow: ScreenElementIR[]; bottomRow: ScreenElementIR[] } => {
  ...
  if (texts.length === 1) {
    return { topRow: [texts[0]], bottomRow: [] };
  }
  const sortedByY = [...texts].sort((a, b) => (a.y ?? 0) - (b.y ?? 0) || (a.x ?? 0) - (b.x ?? 0));
  const minY = sortedByY[0].y ?? 0;
  const maxY = sortedByY[sortedByY.length - 1].y ?? 0;
...
const renderSemanticAccordion = (
  ...
  const detailsContainer = detailsRoot.children?.length === 1 ? detailsRoot.children[0] : detailsRoot;
...
const fallbackScreenFile = (screen: ScreenIR, mappingByNodeId: Map<string, ComponentMappingRule>): FallbackScreenFileResult => {
  ...
  const fileName = ensureTsxName(componentName);
...
const updateAccordionState = (accordionKey: string, expanded: boolean): void => {
  ...
  return {
    file: {
      path: path.posix.join("src", "screens", fileName),
...
export const toDeterministicScreenPath = (screenName: string): string => {
  ...
  return path.posix.join("src", "screens", ensureTsxName(componentName));
};
...
const makeAppFile = (screens: ScreenIR[]): string => {
  const imports = screens
    .map((screen) => {
      const componentName = toComponentName(screen.name);
      return `import ${componentName}Screen from "./screens/${ensureTsxName(componentName).replace(/\.tsx$/, "")}";`;
    })
    .join("\n");

  const routes = screens
    .map((screen) => {
      const routeName = sanitizeFileName(screen.name).toLowerCase();
      const componentName = toComponentName(screen.name);
      return `        <Route path="/${routeName}" element={<${componentName}Screen />} />`;
    })
    .join("\n");
...
const collectSelectOptionLiterals = (source: string): string[] => {
  ...
  while (menuItemMatch) {
    const value = menuItemMatch[1].trim();
...
const validateLlmScreenByExpectation = (
  ...
  if (screenDiagnostics && screenDiagnostics.length > 0) {
    return {
      isValid: false,
      reason: typeScriptRuntime.flattenDiagnosticMessageText(screenDiagnostics[0].messageText, "\n")
    };
  }
...
const hasTopLevelBorderRadiusInCreateTheme = ({
  ...
  const visit = (node: ts.Node): void => {
    ...
    if (
      ...
      if (node.arguments.length > 0 && typeScriptRuntime.isObjectLiteralExpression(node.arguments[0])) {
...
const validateLlmThemeCandidate = ({
  ...
  if (syntaxDiagnostics && syntaxDiagnostics.length > 0) {
    return {
      isValid: false,
      reason: typeScriptRuntime.flattenDiagnosticMessageText(syntaxDiagnostics[0].messageText, "\n")
    };
  }
...
export const generateArtifacts = async ({
  ...
  for (const [nodeId, mapping] of mappingByNodeId.entries()) {
    ...
    if (!mapping.enabled) {
      mappingWarnings.push({
        code: "W_COMPONENT_MAPPING_DISABLED",
        message: `Mapping for node '${nodeId}' is disabled and falls back to deterministic generation`
      });
    }
  }
  ...
  const strictLlmMode = llmCodegenMode === "llm_strict";
  const deterministicMode = llmCodegenMode === "deterministic";
  ...
  const skipLlmEnhancement = llmCodegenMode === "hybrid" && isWeakLlmForCodegen(llmModelName);
        ...
        const llmFile =
          attempt < totalAttempts
            ? await llmClient.generateScreen(screen, ir.tokens, deterministicScreen.path, {
                ...
                repairReason: attempt === 1 ? undefined : lastFailureReason,
