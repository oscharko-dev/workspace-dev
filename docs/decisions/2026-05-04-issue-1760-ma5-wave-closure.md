...
| 3 | Bias controls (CalibraEval position-bias calibration; no length normalization; cross-family panel) | `src/test-intelligence/semantic-judge-panel.ts` | `semantic-judge-panel.test.ts` | `hardLengthNormalizationApplied: false`; cross-family panel (gpt-oss-120b + phi-4-multimodal-poc) | `test:ti-eval` |
