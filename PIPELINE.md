...
- The local full-release profile (`pnpm run release:quality-gates`) and the manual changesets release preflight execute `pnpm run test:coverage` directly. The `dev` and `main` branch gates stay intentionally fast and rely on focused tests plus build/typecheck to avoid blocking normal iteration with deep coverage runs.
