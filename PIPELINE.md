...
- `codegen.generate` emits the generated app theme artifacts under `src/theme/`: canonical `tokens.json`, CSS custom properties in `tokens.css`, and token coverage evidence in `token-report.json`.
