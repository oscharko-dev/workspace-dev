...
| Subprocessor register + cross-border transfer story     | Article 28; GDPR Ch. V | Documented register of LLM gateway, visual sidecar, document-AI, Jira gateway, and operator-defined hook HTTP egress; per-region-pair transfer ADR (SCC / BCR / adequacy decision); CODEOWNERS coupling between register and ADR; `SUBPROCESSOR_REGISTER_VERSION` stamped into every Wave 1 Validation evidence manifest | `docs/dora/subprocessor-register.md`, `docs/dpia/cross-border-transfer.md`, `wave1-validation-evidence-manifest.json` (`subprocessorRegisterVersion`)                              |
...
| Screenshot handling     | Governance JSON artifacts never embed raw screenshot bytes and stamp `rawScreenshotsIncluded: false`; when the production runner persists screenshot files under `visual-captures/`, the adjacent manifest attests those files separately via hashes, paths, and byte counts | `visual-sidecar-result.json`, `visual-captures/manifest.json`, `wave1-poc-evidence-manifest.json`, `export-report.json`, `dry-run-report.json` |
