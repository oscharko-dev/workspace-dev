...
const configuredPort = Number.parseInt(
  process.env.WORKSPACE_DEV_E2E_PORT?.trim() ?? "",
  10,
);
const defaultPort =
...
const runtimeBaseUrl =
  configuredRuntimeBaseUrl ?? `http://127.0.0.1:${String(defaultPort)}`;
...
const UNSUPPORTED_ENVELOPE_PASTE_PAYLOAD = readFileSync(
  UNSUPPORTED_ENVELOPE_FIXTURE_PATH,
  "utf8",
);
const TERMINAL_STATUS_PATTERN = /^(COMPLETED|FAILED|CANCELED)$/;
const TERMINAL_STATUS_CAPTURE_PATTERN =
  /Submit:\s*(COMPLETED|FAILED|CANCELED)/i;
...
export function parseLiveJobPayload(
...
function extractSubmitJobId(value: unknown): string | null {
  ...
  if (
    ...
    value.payload.jobId.trim().length > 0
  ) {
...
function extractJobIdFromPreviewUrl(
...
const SUBMISSION_LOCK_PATH = path.join(
  os.tmpdir(),
  "workspace-dev-playwright-submit.lock",
);
    ...
    try {
      ...
      try {
        ...
        if (Number.isFinite(previousTimestamp)) {
          waitMs = Math.max(
            0,
            SUBMISSION_SPACING_MS - (Date.now() - previousTimestamp),
          );
        }
      } catch {
 ...
 */
export async function withSubmissionRateLimit<T>(
  callback: () => Promise<T>,
): Promise<T> {
 ...
 */
export async function cleanupDeterministicSubmitRoute(
  page: Page,
): Promise<void> {
 ...
 */
export async function openWorkspaceUi(
  page: Page,
  ...
  await expect(
    page.getByRole("heading", { name: "Workspace Dev" }),
  ).toBeVisible();
}
 ...
 */
export async function openInspectorBootstrap(
  page: Page,
  viewport: ViewportSize,
): Promise<void> {
  await page.setViewportSize(viewport);
 ...
 */
export async function simulateInspectorPaste(
  page: Page,
  text: string,
): Promise<void> {
  ...
  await page.evaluate((pasteText) => {
    ...
    const target =
      textareas.find((candidate) => {
        ...
        return (
          labelText.includes("figma json paste target") ||
          labelText.includes("figma clipboard paste target")
        );
      }) ?? textareas[0];
 ...
 */
export async function triggerDeterministicGeneration(
  page: Page,
): Promise<void> {
...
export async function waitForSubmitTerminalStatus(
  ...
  while (Date.now() - startedAt <= timeoutMs) {
    ...
    const runtimeCardText =
      (await runtimeCard.textContent())?.replace(/\s+/g, " ").trim() ?? "";
    ...
    const openInspectorButton = page.getByRole("button", {
      name: "Open Inspector",
    });
    if (
      (await openInspectorButton.count()) > 0 &&
      (await openInspectorButton.isVisible().catch(() => false))
    ) {
      return "COMPLETED";
    }

    const jobStatusText =
      (await page.getByTestId("job-status-card").textContent())
        ?.replace(/\s+/g, " ")
        .trim() ?? "";
    ...
    const waitMs =
      intervalsMs[Math.min(intervalIndex, intervalsMs.length - 1)] ?? 2_000;
    intervalIndex += 1;
    await page.waitForTimeout(waitMs);
  }

  throw new Error(
    `Timed out waiting for terminal submit status after ${String(timeoutMs)}ms.`,
  );
}
...
export async function waitForCompletedSubmitStatus(
  ...
  expect(
    terminalStatus,
    `Expected deterministic flow to complete, but terminal status was ${terminalStatus}`,
  ).toBe("COMPLETED");
}
 ...
 */
export async function openInspectorDialog(
  page: Page,
  label: InspectorDialogLabel,
): Promise<void> {
 ...
 */
export async function selectSecondInspectorFile(
  fileSelector: Locator,
): Promise<string | null> {
  ...
  expect(
    secondValue,
    "Expected second file selector option to have a value",
  ).toBeTruthy();
 ...
 */
export async function collectPreviewNodeIds(
  previewFrame: FrameLocator,
): Promise<string[]> {
 ...
 */
export async function findFirstSyncedNodeId(
  page: Page,
