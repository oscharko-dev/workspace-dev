...
function overlayQuickSetKeyToPercent(
  value: (typeof OVERLAY_QUICK_SET_KEYS)[number],
): number {
    ...
    },
    {
      ...
      entries: [
        { keys: ["?"], description: "Toggle this shortcut help" },
        {
          ...
          description: `Set overlay opacity to ${OVERLAY_QUICK_SET_KEYS.map(
            (value) => `${overlayQuickSetKeyToPercent(value)}%`,
          )
            .join(", ")
            .replace(/, ([^,]*)$/, ", or $1")}`,
        },
...
export function ShortcutHelp({
  ...
  return (
    <div
      ...
      className="sh-overlay absolute inset-0 z-50 flex items-center justify-center sh-overlay-backdrop"
      onClick={(e) => {
    ...
    >
      <div
        ...
        className="sh-panel relative max-h-[80vh] w-full max-w-lg overflow-y-auto rounded-xl border shadow-2xl"
      >
        {/* Header */}
        <div className="sh-header sticky top-0 z-10 flex items-center justify-between border-b px-5 py-3">
          <h3 className="sh-title m-0 text-base font-bold">
            Keyboard Shortcuts
          </h3>
          <button
            ref={closeButtonRef}
            ...
            onClick={onClose}
            className="sh-close-btn cursor-pointer rounded p-1 text-lg leading-none transition hover:opacity-70"
          ...
          {categories.map((category) => (
            <div key={category.title} className="mb-4 last:mb-0">
              <h4
                ...
                className="sh-category-heading m-0 mb-2 rounded px-2 py-1 text-[11px] font-bold uppercase tracking-wider"
              >
                {category.title}
              </h4>
              <div className="flex flex-col gap-1.5">
                {category.entries.map((entry, idx) => (
                  ...
                  >
                    <span className="sh-entry-description text-xs">
                      {entry.description}
                    </span>
                    <span className="flex shrink-0 items-center gap-1">
                      {entry.keys.map((key, ki) => (
                        <kbd
                          key={ki}
                          className="sh-kbd inline-flex min-w-[1.5rem] items-center justify-center rounded border px-1.5 py-0.5 text-[11px] font-mono font-semibold leading-none"
                        >
                          {key}
                        </kbd>
                      ))}
        ...
        {/* Footer hint */}
        <div className="sh-footer border-t px-5 py-2 text-center text-[10px]">
          Press{" "}
          <kbd className="sh-kbd mx-0.5 inline-flex items-center rounded border px-1 py-0 text-[10px] font-mono font-semibold">
            ?
          </kbd>{" "}
          or{" "}
          <kbd className="sh-kbd mx-0.5 inline-flex items-center rounded border px-1 py-0 text-[10px] font-mono font-semibold">
