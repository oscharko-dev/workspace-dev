...
type ExportMode = "clipboard" | "upload";
...
interface ExportSelectionMessage {
  type: "export-selection";
}

interface UploadToLocalMessage {
...
type UiMessage = ExportSelectionMessage | UploadToLocalMessage | CloseMessage;
...
function createSelectionUnit(
  ...
  const document: Record<string, unknown> =
    isRecord(rawDocument) && Object.keys(rawDocument).length > 0
      ? rawDocument
      : {
          id: node.id,
          type: node.type,
...
function compareNodesForExport(left: SceneNode, right: SceneNode): number {
